...
class Visualizer:
    ...
    def draw_norm_img(self, norm_img):
        self.image[0:norm_img.shape[0], 0:norm_img.shape[1],...] = np.repeat(norm_img[:, :, np.newaxis], 3, axis=2)
    ...
    def draw_points(self,
        ...
        print(len(points))
        cv2.circle(self.image, self._convert_pt(points[38]), size, color, cv2.FILLED)
        cv2.circle(self.image, self._convert_pt(points[40]), size, color, cv2.FILLED)
    ...
    def draw_3d_line(self,
        ...
        #cv2.circle(self.image, pt0, 4, (0,0,255), cv2.FILLED)
    ...
    def draw_eye_blink(self, face: Face):
        cv2.putText(self.image, "(%0.2f, %0.2f)" % (
                    face.leye.opened, face.leye.closed), (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        cv2.putText(self.image, "(%0.2f, %0.2f)" % (
                    face.reye.opened, face.reye.closed), (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)